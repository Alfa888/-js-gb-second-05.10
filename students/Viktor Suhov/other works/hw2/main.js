...
//const renderGoodItem = (title='Cup', price = 25) => {
...
const renderGoodsList = (list) => {
    ...
    const renderGoodsList = (list) => {
        for (let item of list) {
            document.querySelector ('.goods-list').insertAdjacentHTML ("beforeend", renderGoodsItem 
    ...
    const items = ['Notebook', 'Display', 'Keyboards', 'Mouse', 'Phones', 'Router', 'USB-camera', 'Gamepad'];
    const price = [1000, 200, 20, 10, 25, 30, 18, 24];
    ...
    function createProduct(i) {
        return {
            ...
            createTemplate: function () {
                ...
                <h3>${this.name}</h3>
                <p>${this.price} $</p>
                <button class = "buy-btn" data-id = "${this.id}" data-name = "${this.name}"
                data-image = "${this.img}" data-price = "${this.price}">Купить</button>
                </div>`
            }
            add: function () {
                this.quantity++
...
const image = 'https://placehold.it/200x150';
const cartImage = 'https://placehold.it/100x80';
const items = ['Notebook', 'Display', 'Keyboard', 'Mouse', 'Phones', 'Router', 'USB-camera', 'Gamepad', 'Ipad'];
const prices = [1000, 200, 20, 10, 25, 30, 18, 24, 222];
